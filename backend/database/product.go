...
func init() {
	...
	// for _, p := range productList {
	// 	err := qry.InsertProduct(p) // InsertProduct function Postgres compatible হতে হবে ($1, $2, …)
	// 	if err != nil {
	// 		fmt.Println("❌ Insert error for product", p.ID, ":", err)
	// 	} else {
	// 		fmt.Println("✅ Inserted product", p.ID)
	// 	}
	// }
